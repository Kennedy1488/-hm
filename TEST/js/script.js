...
function shuffleArray(array) {
    for (var i = array.length - 1; i > 0; i--) {
        var j = Math.floor(Math.random() * (i + 1)); // Генерируем случайный индекс  от 0 до i
        [array[i], array[j]] = [array[j], array[i]]; // Обмен местами элементов  массива
    }
    return array;
}
...
function displayQuestion() {
    var questionElement = document.getElementById("question"); // Получаем блоккуда размещать вопрос
    questionElement.textContent = "Вопрос " + (currentQuestion + 1) + ": " +
        questions[currentQuestion].question; // Размещаем вопрос
    var optionsElement = document.getElementById("options"); // Получаем блоки кнопок
    optionsElement.innerHTML = "";
    // Перемешиваем варианты ответов
    var shuffledOptions = shuffleArray(questions[currentQuestion].options);
    for (var i = 0; i < shuffledOptions.length; i++) { // Создаем кнопки с вариантами ответов и привязываем к ним функцию nextQuestion
